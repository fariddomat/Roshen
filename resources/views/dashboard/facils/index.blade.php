        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header card-header d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('dashboard.facils.create') }}" class="btn btn-icon btn-info mr-1">@lang('site.create')
                    ...
                    @else
                        <div class="table-responsive">
                            <table class="table table-striped table-scrollable">
                                ...
                                <tbody>
                                    @foreach ($facils as $index => $facil)
                                        <tr dir="rtl" style=" text-align: right;">
                                            <th scope="row">{{ $index + 1 }}</th>
                                            <td dir="rtl">{{ $facil->name }}</td>
                                            <td><img src="{{ asset($facil->icon) }}" class="avatar" alt=""></td>
                                            <td class="form-group">

                                                <a href="{{ route('dashboard.facils.edit', $facil->id) }}" type="button"
                                                    class="btn btn-icon btn-warning mr-1"
                                                    style="  min-width: 100px;">@lang('site.edit') <i class="fa fa-edit"
                                                        style="position: relative;"></i></a>

                                                <form action="{{ route('dashboard.facils.destroy', $facil->id) }}"
